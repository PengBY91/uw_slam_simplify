...
Scope note: this package now contains only the **offline recording path** (simulator → canonical
MCAP bag → C++ replay). The realtime closed-loop line (realtime ROS session, fault injection, task
scoring, gates, scripted pilot, ArduSub SITL bridge) lived here historically and was removed from
main; the full snapshot is preserved on the `archive/rov-realtime-line2` branch.

## What's real vs. not tested here

This machine has no HoloOcean/Unreal install, so `holoocean_driver.py`'s `HoloOceanSession` (the
part that calls into HoloOcean itself) is written but not exercised here. Everything else IS tested
(`pytest`):
...
- `calibrate_camera.py` — chessboard stereo calibration from a recorded bag → `configs/rig/*.yaml`
  (see `docs/calibration/` at repo root for mounting constraints and acceptance).

## Tools

Scripts under `tools/` are not part of the installed package; run them with `.venv/bin/python`.

- `tools/sonar_stats.py` — sonar image statistics (Rayleigh noise floor, signal-to-clutter,
...
- `../../tools/calib/imu_allan.py` — Allan-deviation IMU noise identification -> rig
