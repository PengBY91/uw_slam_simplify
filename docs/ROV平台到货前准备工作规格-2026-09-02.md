...
> 状态：门禁中（2026-09-03）。PREP-B-06 的控制点配置与指标 API 已实现；真实水池长宽深、池壁/池底材质、标记位置和可用日期仍未确认。尺寸确认前只允许维护 `pool_example.yaml` 模板，不得声明真实水池关卡已完成。
...
> 状态：第 3 周已交付"定义"部分（2026-09-03）——定义短文 `docs/sparse-control-point-evaluation-2026-09-03.md`；scenario YAML 新字段 `control_points`（`tag`/`position_m`/`size_m`/`reflectivity_class`，`reflectivity_class` 沿用 `adapters/holoocean/scenarios/*.yaml` 的 `acoustic_reflectivity_class` 词表，同一份标记表可同时驱动 UE5 关卡和本指标）落进 `include/runtime/config.hpp` 的 `ScenarioControlPoint` + `src/runtime/config.cpp` 解析（重复 tag／缺字段／size ≤ 0 加载期拒绝），模板 `configs/scenario/pool_example.yaml`；指标 `include/evaluation/control_point_metrics.hpp` + `.cpp`（逐观测误差表 + RMSE/mean/p95/max + unmatched/unknown-tag/coverage 三个计数，默认不做 Umeyama 对齐、对齐要求 3 个不同标记）。13 条新单测（评测 10、config 3）全绿。**未做**（第 4 周起）：接进 `replay_demo` 与 run manifest、人工标注观测的文件格式、CFAR + 最近邻自动关联，以及规格验收本身（RMSE 与全轨迹 ATE 相关系数 > 0.8，需要 PREP-A-08 水池关卡）。
...
下图只画**跨工作包**依赖（包内依赖省略），边全部来自各任务自己的「依赖」行。
两个需要注意的地方：**PREP-B-01 是第 4 周的阻断项**——它扇出到 B-02/B-03/B-08 和
C-03 W5；**PREP-A-13 设备伪装层扇入最多**，六条来源里任何一条没到位，它就只能交付
部分路径（这也是它按周拆分的原因）。
